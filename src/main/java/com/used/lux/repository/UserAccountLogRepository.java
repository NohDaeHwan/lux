...
public interface UserAccountLogRepository extends JpaRepository<UserAccountLog, Long> {
    @Query(nativeQuery = true,value ="select * from user_account_dto where user_email=:userEmail",
            countQuery="select count(*) from user_account_dto where user_email=:userEmail")
