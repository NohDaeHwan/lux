...
public class AuctionController {
    ...
    // AuctionResponse.from(auctionService.auctionFind(id));
    ...
    // @PageableDefault(size = 30, sort = "created_at", direction =
    // Sort.Direction.DESC) Pageable pageable
    // ) {
    // Page<AuctionsResponse> auctions =
    // auctionService.auctionResultFind(pageable).map(AuctionsResponse::from);
    ...
    // AuctionResponse auction =
    // AuctionResponse.from(auctionService.resultFind(id));
    // return ResponseEntity.status(HttpStatus.OK).body(auction); // 종료된 경매 상세 페이지
    // }
    //
    // @GetMapping("/info")
    // public ResponseEntity<Integer> auctionInfo() {
    // return ResponseEntity.status(HttpStatus.OK).body(1); // 경매 정보 페이지 View를 보여줘야
    // 함
    // }
    //
    ...
    // }
    // Integer result = auctionService.auctionUpdate(auctionId,
    // auctionBidRequest.toDto(), principal.getUsername());
